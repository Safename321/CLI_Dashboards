...
- **Credit the poster, don't rank them against their source (added 2026-09-08).** When a comment engages with a quote or stat the poster surfaced, don't frame it as the source outshining the poster (e.g., "a sharper diagnosis than the post itself"). Instead, credit the poster for surfacing it — "I connected with [the quote] the moment I read it — thanks for pulling that into the post, [name]" — and let the substance of the observation carry the sharpness on its own. The goal is generous engagement with what the poster chose to highlight, not a comparison of who said it better.
...
3. **@-mention people the original post itself names** (co-authors, colleagues, guests thanked in the post) — inline, first mention, using LinkedIn's actual mention syntax, `@`, never `#` (2026-09-11 correction: LinkedIn tags people with @, not hashtags — e.g. "Dr. Arun Sanyal @ArunSanyal"). Only tag people already named in the source post; do not invent tags for people not mentioned.
4. **Embed the CLI URL in-sentence, naturally, always with the `https://` prefix** — never as a tacked-on "(Related read: connectiveleadership.com)" parenthetical at the end, and never a bare domain (2026-09-11 correction). It should read as part of the sentence's own claim, e.g. "...the translation layer we spend our time in at https://connectiveleadership.com/#how."
...
## Part 16 — The 200-Character LinkedIn Connection Note Method (added 2026-09-10, corrected 2026-09-10)
...
2. **Search the whole profile for genuine common ground — the search comes first, composition follows.** Read the About section, recent posts (plural — look across several, not just the latest), Experience, Education, and any visible shared connections or groups. Find the most genuine, most specific point of overlap between them and CLI's actual work.
3. **Let the composition take whatever form the common ground actually calls for — quote, paraphrase, or stated fact.** If they've said the thing themselves, in their own words, and that line precisely captures the overlap — quote it directly. That's often the strongest option, not something to avoid. Paraphrase or name a pattern instead only when that's the more accurate fit: when the real common ground is a throughline across several posts rather than any single line, or when no one quote says it as precisely as describing the pattern does. The rule is the search; the form is chosen to match what was actually found, never decided in advance.
4. **Never fabricate shared background.** Do not assume a shared alma mater, city, or employer from a profile detail unless it's actually confirmed to overlap with the sender's own background. An unverified "fellow [school/city]" reads as hollow at best and gets caught at worst.
5. **State the overlap, then a soft close.** One sentence naming the specific, genuine point of overlap; one short close ("Would welcome connecting" or similar) — no pitch, no flattery, no "great post."
6. **Target ~150-190 characters**, leaving margin under the 200 cap rather than writing to the limit and trimming under pressure.
