...
- **NEXT (resume here):** as agents finish — write src/dashboards/index.jsx registry with real paths, `vite build`, fix integration errors, Playwright smoke across all views, commit (50% milestone at ~half ported), wire the AI mentor + verify proxy/auth/rate-limit live (75%), add the §6 unit + e2e tests, satisfy all §8 acceptance criteria (100%). Build the two interactive tools (Fill Jobs, Mgmt Challenges) natively per §4.3b. Add README documenting demo creds + serving contexts.
